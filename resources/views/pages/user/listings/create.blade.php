    ...
    <form action="{{ isset($listing) ? route('user.listings.update', $listing) : route('user.listings.store') }}"
        ...
        <div class="panel rounded-[24px] overflow-hidden">
            ...
            <div class="p-6">
                @if(isset($listing) && $listing->images->isNotEmpty())
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        @foreach($listing->images as $img)
                            <div x-data="{ deleted: false }" x-show="!deleted" class="relative aspect-square rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 shadow-sm group">
                                ...
                                {{-- Input caché qui s'ajoute au formulaire quand l'image est supprimée --}}
                ...
                <input x-ref="imagesInput" type="file" name="images[]" multiple accept="image/*" class="hidden"
                       @change="previewImages($event)">

                <div class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-[16px] hover:border-gold/50 hover:bg-gold/5 transition-all text-center px-4"
                     @dragover.prevent
