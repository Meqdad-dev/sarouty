        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-8">
            @foreach($plans as $plan)
                @php
                    ...
                    $buttonStyle = 'background:' . $theme['button'] . '; color:#fff;';
                @endphp

                <div class="plan-card rounded-2xl border overflow-hidden flex flex-col"
                     style="background:#fff; border-color: {{ $isRecommended ? $theme['hex'] : $theme['border'] }}; box-shadow: {{ $isRecommended ? '0 24px 64px ' . $theme['glow'] : '0 16px 40px rgba(26, 20, 16, 0.08)' }};">

                    {{-- Recommended badge --}}
                    @if($isRecommended)
                        <div class="text-white text-center text-xs font-bold py-2 uppercase tracking-widest"
                             style="background: linear-gradient(90deg, {{ $theme['hex'] }} 0%, {{ $theme['button_hover'] }} 100%);">
                            Le plus populaire
                        </div>
                    @else
                        <div class="h-1.5" style="background: {{ $theme['hex'] }};"></div>
                    @endif

                    <div class="p-7 flex flex-col flex-1">
                        {{-- Plan header --}}
                        <div class="mb-6">
                            <div class="inline-flex mb-3 items-center justify-center rounded-2xl border px-4 py-3"
                                 style="background: {{ $theme['soft'] }}; border-color: {{ $theme['border'] }}; color: {{ $theme['button'] }}; box-shadow: 0 14px 30px {{ $theme['glow'] }};">
                            ...
                            <div class="flex items-center gap-2 mb-2">
                                <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em]"
                            ...
                            <h3 class="font-display text-2xl font-bold" style="color: {{ $theme['text'] }};">{{ $plan->name }}</h3>
                        </div>

                        {{-- Price --}}
                        <div class="mb-6 pb-6 border-b" style="border-color: {{ $theme['border'] }};">
                            @if($plan->price == 0)
                                <div class="font-display text-4xl font-bold" style="color: {{ $theme['text'] }};">Gratuit</div>
                                ...
                                <div>
                                    ...
                                    <div x-show="billing !== 'annuel'">
                                        <div class="flex items-end gap-1">
                                            <span class="font-display text-4xl font-bold" style="color: {{ $theme['text'] }};">{{ $priceM }}</span>
                                    ...
                                    <div x-show="billing === 'annuel'" style="display:none">
                                        <div class="flex items-end gap-1">
                                            <span class="font-display text-4xl font-bold" style="color: {{ $theme['text'] }};">{{ $priceA }}</span>
                            ...
                            @if($plan->features)
                                @foreach($plan->features as $feature)
                                    <li class="flex items-start gap-2.5 text-sm text-gray-700">
                                        <svg class="feature-check" style="color: {{ $theme['button'] }};" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @auth
                            ...
                            @else
                                <a href="{{ route('user.checkout.form', ['plan' => $plan->slug]) }}"
                                   class="w-full py-3 rounded-xl text-sm font-semibold transition-all shadow-sm flex items-center justify-center gap-2" style="{{ $buttonStyle }}">
                                    Choisir ce plan
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            ...
                            @else
                                <a href="{{ route('register') }}"
                                   class="block w-full py-3 rounded-xl text-sm font-semibold text-center transition-all shadow-sm flex items-center justify-center gap-2" style="{{ $buttonStyle }}">
